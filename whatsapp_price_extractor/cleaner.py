...
#     if empty_removed > 0:
#         print(f" {empty_removed} messages vides supprimés")
...
def filtrer_pertinence_porcs(df):
    """
    Version corrigée : accepte tout message contenant un animal ou un nombre suivi de FCFA
    """
    mask = df["message"].str.lower().apply(
        lambda x: bool(re.search(r'(porc|truie|verrat|porcelet|cochon|\d{4,6}\s*(?:fcfa|f))', x))
    )
    df_filtre = df[mask]
    return df_filtre
...
def extraire_entites_porcines(message):
    message_str = str(message).lower()
    entites = {'animal_type': None, 'produit_type': None, 'unite': None, 'transaction_type': None}
    if re.search(r'\b(porc|cochon)\b', message_str): entites['animal_type'] = 'porc'
    elif re.search(r'\btruie\b', message_str): entites['animal_type'] = 'truie'
    elif re.search(r'\bverrat\b', message_str): entites['animal_type'] = 'verrat'
    elif re.search(r'\bporcelet\b', message_str): entites['animal_type'] = 'porcelet'
    if re.search(r'\b(kg|kilo|kilogramme)\b', message_str): entites['unite'] = 'kg'
    elif re.search(r'\b(sac|tonne|têtes?)\b', message_str): entites['unite'] = 'sac'
    if re.search(r'\b(vendre|vente|disponible|prix)\b', message_str): entites['transaction_type'] = 'vente'
    elif re.search(r'\b(acheter|besoin|cherche|recherche)\b', message_str): entites['transaction_type'] = 'achat'
    if entites['animal_type']: entites['produit_type'] = 'animal_vivant'
    return entites

def categoriser_message(message):
    msg = str(message).lower()
    # nouveau pattern prix
    if re.search(r'\d{4,6}\s*(?:fcfa|f|frs?)?', msg): return 'avec_prix'
    if any(mot in msg for mot in ['vendre', 'vente', 'disponible']): return 'offre_vente'
    if any(mot in msg for mot in ['acheter', 'besoin', 'cherche', 'recherche']): return 'demande_achat'
    if any(mot in msg for mot in ['conseil', 'aide', 'problème', 'malade', 'suggestion']): return 'conseil'
    if any(mot in msg for mot in ['contact', 'numéro', 'téléphone', 'appelez', 'whatsapp']): return 'contact'
    if any(mot in msg for mot in ['félicitation', 'bravo', 'merci']): return 'félicitations'
    return 'autre' 
def nettoyer_messages(input_csv, output_csv="messages_final.csv"):
    """
    Nettoie le CSV brut exporté par parser.py
